...
public class SonataSimulation {
	...
	public static void main(String[] args) {
		...
		System.out.println(myCar.speed);
		System.out.println(myCar.carName);
		myCar = null; // 42→43번으로 갈 때 Candidate상태에 빠진다.
		// 38번에 생성된 객체는 쓰레기 값으로 인식되어 자원을 회수 당하게 된다. ﻿→ 그시점이 인스턴스화하고 null로 하고 새로운 인스턴스화하기 직전의 상태 때!
		myCar = new Sonata(); // 널포인트 입셉션 안당하려면 인스턴스화
		System.out.println("null 초기화 후의 비교" + myCar);
		System.out.println(myCar.speed);
		System.out.println(myCar.carName);
	}
}
