...
<%
	/* 로그인 성공하면 session 값 댕겨오기 / 서버에서 결정된 값 넘어오기 */
	int s_cnt = 0;
...
<script type="text/javascript">
	...
	function receiveMemoList() {
		$("#dg_receiveMemoList").datagrid({
			...
			,onSelect : function(index, row) { //내용보기 n->y (전체가아닌 그건에대해서만)
				gm_no = row.NO;//데이터그리드 선택시 해당로우의 NO담기
			}
		});
		$("#d_member").hide();
		//after
	...
	function memoContent(){
		console.log("쪽지내용보기 호출 성공")
		$("#dlg_memoContent").dialog({
			title : "쪽지내용확인",
			// 쪽지내용을 DB에서 꼭 가져와야 하나요?
			// 목록을 가져올때 가지고 있다가 출력해도 되지 않을까?(오라클 경유를 안해도 됨)
			// 그럼 너라면 1번? 2번 중 몇번 선택할래??   ((쪽지만보고끝?N을 Y로업뎃?))
			// --기준--:쪽지내용 확인 후 read_yn을 Y로 업데이트해야하니까...난 1번선택할래~!
			// 한건을 조회한 후에 그 자료구조가 null이 아닐때만 업데이트 해야 됨
			// 만일 500이 일어나면 진행이 안되야 됨
			// 이 기능을 구현하는데 있어서 컨트롤계층에 메서드부터 호출이 되어야 하는건지? 아니면
			// 로직클래스에 메서드 호출만으로도 충분한지를 고민해 보기 ﻿→ 객체주입관계와 메서드 선언을 결정함
			// 설계자는 기능 담당자가 구현해야하는 페이지이름과 메서드, 그리고 파라미터와 리턴타입 모두를
			// 다 정해놓고 담당자는 메서드안에 기능구현만 하도록 할 것!
